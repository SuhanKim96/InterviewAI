...
export default function DocumentUpload({ lang, onDone, onBack }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-zinc-200 p-7">
      {onBack && (
      ...
      <h2 className="text-base font-semibold text-zinc-900 mb-1">{t.uploadTitle}</h2>
      <p className="text-sm text-zinc-500 mb-6">{t.uploadDesc}</p>
      ...
      >
        ...
        />
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-7 h-7 mx-auto mb-2 text-zinc-400">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
        </svg>
        <p className="text-sm font-medium text-zinc-600">{t.dropZoneText}</p>
        <p className="text-xs text-zinc-400 mt-1">{t.dropZoneHint}</p>
      </div>
        ...
        <div className="space-y-1.5 mb-4">
          {files.map((f) => (
            <div key={f.name} className="flex items-center gap-2 px-3 py-2 bg-indigo-50 rounded-lg">
              ...
              <button onClick={() => removeFile(f.name)} className="text-zinc-300 hover:text-red-400 transition-colors ml-1">
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-zinc-700 mb-1">
          {t.portfolioLabel} <span className="text-zinc-400 font-normal">{t.optional}</span>
        </label>
        <textarea
          rows={4}
          value={portfolioText}
          ...
          className="w-full border border-zinc-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 resize-none"
        />
      </div>
      ...
      {result && (
        <div className="border border-zinc-200 rounded-lg p-4 mb-5 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-zinc-800">{t.analysisDone}</p>
            {result.indexed_files.length > 0 && (
              <p className="text-xs text-zinc-500 mt-0.5">{result.indexed_files.join(', ')}</p>
            )}
            {result.github_repos.length > 0 && (
              <p className="text-xs text-zinc-500">GitHub: {result.github_repos.join(', ')}</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3">
        ...
        {result && (
          <button
            onClick={onDone}
            className="px-5 py-2.5 border border-zinc-300 text-zinc-700 text-sm font-semibold rounded-lg hover:bg-zinc-50 transition-colors"
          >
            {t.nextStep}
          </button>
        )}
