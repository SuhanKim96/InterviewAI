          ...
          <div key={s.key} className="flex items-center">
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-md">
              <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0
                ${done ? 'bg-indigo-600 text-white' : active ? 'bg-indigo-100 text-indigo-700 ring-1 ring-indigo-400' : 'bg-gray-100 text-gray-400'}`}>
                {done ? (
              ...
              <span className={`text-xs hidden sm:block ${active ? 'text-gray-800 font-medium' : done ? 'text-indigo-600' : 'text-gray-400'}`}>
                {s.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`w-5 h-px ${i < current ? 'bg-indigo-300' : 'bg-gray-200'}`} />
            )}
          </div>
        )
      })}
