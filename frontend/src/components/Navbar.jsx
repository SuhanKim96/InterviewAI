...
export default function Navbar({ step, onHome, lang, onLangToggle }) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-zinc-200 h-14 flex items-center px-6">
      <div
        onClick={onHome}
      ...
      >
        ...
        <span className="font-semibold text-zinc-900 text-sm">InterviewAI</span>
      </div>
      <div className="ml-auto flex items-center gap-3">
        {showStepper && <Stepper step={step} lang={lang} />}
        <button
          onClick={onLangToggle}
          className="text-xs font-semibold text-zinc-500 hover:text-indigo-600 border border-zinc-200 hover:border-indigo-300 rounded-md px-2.5 py-1 transition-colors"
        >
          {T[lang].langToggle}
        </button>
      </div>
