...
export default function DocumentsPage({ lang, onBack }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm(t.confirmDeleteDocs)) return
    setDeleting(true)
  ...
  return (
    <div>
      <div className="py-8 border-b border-zinc-100 mb-6">
        ...
        <h2 className="text-xl font-bold text-zinc-900 mb-1">{t.docsTitle}</h2>
        <p className="text-sm text-zinc-500">{t.docsDesc}</p>
      </div>

      {loading ? (
        <div className="text-sm text-zinc-400 py-12 text-center">{t.loadingHistory}</div>
      ) : sources.length === 0 && !showUpload ? (
        <div className="bg-white rounded-lg border border-zinc-200 p-8 text-center">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-10 h-10 mx-auto mb-3 text-zinc-300">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          <p className="text-sm text-zinc-500 mb-4">{t.noDocsYet}</p>
          <button
        ...
        <>
          {sources.length > 0 && (
            <div className="bg-white rounded-lg border border-zinc-200 mb-5">
              <div className="px-5 py-4 border-b border-zinc-100">
                <p className="text-sm font-semibold text-zinc-700">{t.analyzedDocs}</p>
              </div>
              <div className="divide-y divide-zinc-100">
                {sources.map((s, i) => (
                  <div key={i} className="px-5 py-3 flex items-center gap-3">
                    ...
                    <span className="text-sm text-zinc-700">{formatSource(s, t.sourceLabels)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-3 mb-6">
            <button
              ...
              className="px-4 py-2.5 border border-indigo-300 text-indigo-600 text-sm font-semibold rounded-lg hover:bg-indigo-50 transition-colors"
            >
            ...
            <button
              ...
              disabled={deleting}
              className="px-4 py-2.5 border border-red-200 text-red-500 text-sm font-semibold rounded-lg hover:bg-red-50 disabled:opacity-50 transition-colors"
            >
